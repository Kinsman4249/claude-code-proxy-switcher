...
# fit the GPU: which model-profiles/*.sh fragment to load, which quant to
# download, VRAM/context sizing, the optional VRAM-headroom tradeoffs, and
# speculative-decoding draft length. Sets PROFILE_FILE, PROFILE_NAME, and
# the other model-profile globals (N_LAYERS, KV_MODEL, SPEC_MODE, etc) as a
# side effect of sourcing the chosen profile - later install.d/*.sh files
# (model download, launcher generation) rely on those being set here first.
...
# (quant -> VRAM/context needs QUANT_WEIGHT_MIB, headroom needs N_LAYERS
# from the profile, spec decoding needs SPEC_MODE from the profile).
