...
Run from inside your clone (wherever the Quickstart's `git clone` put it). Re-run `install.sh` afterward if the update touched anything you'd want re-applied (new prompts, changed defaults) - it's always safe to re-run, see above.
