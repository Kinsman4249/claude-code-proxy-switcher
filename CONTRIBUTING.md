# Contributing to claude-code-proxy-switcher
...
- Your runtime environment (distro, desktop environment, Distrobox version, Ollama version)
...
3. **Test it.** See "Manual verification" in the README for how to verify your change locally.
4. **Lint it.** Run `shellcheck` against any shell scripts you touched.
...
- **No new dependencies** without strong justification - the appeal of this project is the small, predictable surface area.
- **No telemetry, ever.** This project must not phone home.
- **No API keys, ever, in any config this project generates.** The whole point of this project is that local mode never touches billed cloud paths. Any change that reintroduces an implicit cloud fallback needs to be called out explicitly in the PR description and default to off.
